...
from augment import ImageAugmentation
...
class ImageGenerator():
    ...
    def __init__(self, dataformat):
        self.base_path = "Y:\\UHMH\\image-generator\\image-generator\\"
    ...
    def place_object(self, background, background_roi, objects, MODES):
        ...
        if distractor_decission < 0.2: # 10 percent chance that a distractor is added 
        ...
        for iter in range(len(objects)):
            ...
            while isValid == False: # TODO: here we can do some optimization -> do we have to augment the object completely new every time it does not fit into the background? 
                ...
                # plt.subplot(1, 2, 2)
                # plt.imshow(combined_mask_check)
                # plt.title('Combined Mask Check')
                # plt.show()
                ...
                if MODE == "POISSON_NORMAL" or MODE == "POISSON_MIXED":
                    # TODO: when looking at th blending done by poisson it can be seen that the border is too much (eg pen can barely be seen) 
                    # I do not know how to solve this yet... (maybe implement poisson yourself)
    ...
    def main(self, input_data):
        ...
        dataFormat = mapping_outputs.get(input_data.output, None)

        imageGenerator = ImageGenerator(dataFormat)
        SETTING = 1
        start = time.time()
...
if __name__ == "__main__":
    ...
    if imageGenerator.dataFormat == "pascal_voc":
        ...
        imageGenerator.dataSplit.split_data_coco(0.6,0.3,0.1, ["Pen", "Box"], "implementation\\result")
    end = time.time()
